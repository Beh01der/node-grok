...
function GrokPattern(expression, id) {
    ...
    // TODO: right now both parse and parseSync are really synchronous, perhaps worth cleaning up
    // TODO: remove dependencies on fs and async; instead use patterns passed directly to load() from arrays/memory
    t.parse = function(str, next) {
        if (!t.regexp) {
            t.regexp = XRegExp(t.resolved);
        }
        var result = XRegExp.exec(str, t.regexp);
        // TODO: see how to handle errors from XRegExp
        var err = null;
